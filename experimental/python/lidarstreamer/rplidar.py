...
#        self.baudrate = 115200 # Setting baud to lower value to see if that helps with data transfer
    ...
    def _read_descriptor(self):
        ...
        print("Descriptor: %s" % binascii.hexlify(descriptor))
...
#            raise RPLidarException('Wrong response data type')
    ...
    def iter_measurments(self, force=True):
        ...
        while True:
            raw = self._read_response(dsize)
#            print("Scan Response: %s" % binascii.hexlify(raw))
            self.logger.debug('Recieved scan response: %s' % raw)
            yield _process_scan(raw)

    def iter_scans(self, min_len=5, force=True):
